...
export default function ConvertToTransactionButton({ transaction, onConverted }) {
  ...
  const handleConvert = async () => {
    ...
    await base44.functions.invoke("updateTransaction", {
      ...
      data: {
        ...
        earnest_money_deadline: earnestMoneyDeadline,
        phase: 3,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg rounded-2xl shadow-2xl p-6 space-y-5 overflow-y-auto max-h-[90vh]"
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-base font-semibold" style={{ color: "var(--text-primary)" }}>Convert Listing → Transaction</h3>
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          <Button size="sm" onClick={handleConvert} disabled={saving || !buyers.filter(Boolean).length}
            ...
            {saving ? "Converting…" : "Convert to Transaction"}
          </Button>
        </div>
      </div>
    </div>
