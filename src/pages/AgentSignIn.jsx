...
export default function AgentSignIn() {
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      ...
      } else if (role === "admin" || role === "owner" || role === "tc" || role === "tc_lead") {
  ...
  const handleSignIn = () => {
    base44.auth.redirectToLogin(window.location.origin + "/agent/submit-transaction");
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/40 flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl border border-slate-100 p-8 text-center">
        ...
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Agent Sign In</h1>
        <p className="text-sm text-slate-500 mb-8">
          Sign in with your agent account to submit a transaction.
        </p>
        ...
        <p className="text-xs text-slate-400 mt-5">
          Not an agent?{" "}
          <a href="/tc-signin" className="text-blue-600 hover:underline font-medium">TC sign in →</a>
        </p>
      </div>
    </div>
  );
